...
public class Quicture extends JPanel {
    ...
    private void refresh(){
        pixelsZ = new float[getWidth()][getHeight()];
    ...
    public void setPixel(int x, int y, double z, Color color){
        ...
        if (pixelsZ[x][y] >= z && z > 0 && z < 1) {
            pixelsZ[x][y] = (float)z;
            ((DataBufferInt) picture.getRaster().getDataBuffer()).getData()[y*getWidth() + x] = color.getRGB();
        }
    }
    ...
    public void paintComponent(Graphics g){
        ...
        for (int i = 0; i < quomponents.size(); i++)
            quomponents.get(i).paint(this, cameraOld);
