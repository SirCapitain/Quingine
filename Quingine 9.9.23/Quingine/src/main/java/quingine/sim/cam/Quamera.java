...
public class Quamera {
    ...
    public ArrayList<Quisition[]> clipZPlanes(Quisition[] points){
        ...
        for (int i = 0; i < triangles.size(); i++) {
            clip = Math3D.clipObject(triangles.get(i), new Quisition(0, 0, zFar), new Quisition(0, 0, -1));
            if (clip == null)
                continue;
     ...
     * @param q the current quicture being used.
     * @return an arraylist of an array of newly clipped points
     */
    public ArrayList<Quisition[]> clipScreenEdges(ArrayList<Quisition[]> triangles, Quicture q){
    ...
    public Quisition perspective(Quicture q, Quisition pos){
        ...
        newPos.setPos(
                (q.getHeight()/(double)q.getWidth())*(newPos.x/Math.tan(Math.toRadians(fov*.5)))*newPos.w,
                (newPos.y/(Math.tan(Math.toRadians(fov*.5))))*newPos.w,
