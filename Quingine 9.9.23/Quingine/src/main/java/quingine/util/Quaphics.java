...
public class Quaphics {
     ...
     */
    public static void drawPolygon(Quisition[] points, Quicture quicture, Color color, double brightness){
        ...
        for (int i = 0; i < points.length; i++) {
            ...
            if (dx == 0) {//Vertical
                ...
                for (int y = y1; y != y2; y += yInc)
                    quicture.setPixel(x1, y, -1/zEquation[2]*(zEquation[0]*quicture.getMatrixPosX(x1)+zEquation[1]*quicture.getMatrixPosY(y)+zEquation[3]), colorN);
                continue;
            }
            if (dy == 0) {//Horizontal
                ...
                for (int x = x1; x != x2; x += xInc)
                    quicture.setPixel(x, y1, -1/zEquation[2]*(zEquation[0]*quicture.getMatrixPosX(x)+zEquation[1]*quicture.getMatrixPosY(y1)+zEquation[3]), colorN);
                continue;
            }
            double slope = dy / dx;
            ...
            if (Math.abs(slope) < 1) {//More Horizontal
                ...
                for (int x = x1; x != x2; x += xInc) {
                    double y = x * slope + b;
                    quicture.setPixel(x, (int) Math.round(y), -1/zEquation[2]*(zEquation[0]*quicture.getMatrixPosX(x)+zEquation[1]*quicture.getMatrixPosY(y)+zEquation[3]), colorN);
                }
                continue;
            }
            if (Math.abs(slope) >= 1){//More Vertical
                ...
                for (int y = y1; y != y2; y+=yInc) {
                    double x = (y-b)/slope;
                    quicture.setPixel((int)Math.round(x), y, -1/zEquation[2]*(zEquation[0]*quicture.getMatrixPosX(x)+zEquation[1]*quicture.getMatrixPosY(y)+zEquation[3]), colorN);
                }
            }
        }
    }
     ...
     * @param quicture the quicture to draw on.
     */
    public static void fillPolygon(Quisition[] points, Quicture quicture, Color color, double brightness) {
        Color colorN = updateColor(color, brightness);
        ...
        for (int y = yMin; y <= yMax; y++) {
            ...
            if (xPoints.size() > 1) {
                ...
                for (int x = xPoints.get(0); x <= xPoints.get(1); x++)
                    quicture.setPixel(x, y, -1/zEquation[2]*(zEquation[0]*quicture.getMatrixPosX(x)+zEquation[1]*quicture.getMatrixPosY(y)+zEquation[3]), colorN);
            }
        }
    }
     ...
     */
    public static void fillTri(Quisition[] vertices, Quicture q, Color color, double brightness) {
        Color colorN = updateColor(color, brightness);
     ...
     */
    public static void drawImageTri(Quisition[] vertices, Quicture q, BufferedImage image, double brightness){
        //Draw from top to bottom.
        Quisition[] points = new Quisition[]{vertices[0], vertices[1], vertices[2]};
        if (points[1].y > points[0].y)
            points = new Quisition[]{points[1], points[0], points[2]};
        ...
        for (int i = 0; i < 2; i++) {
            ...
            for (int y = sy; y < ey; y++) {
                ...
                for (int x = startX; x < endX; x++) {
                    ...
                    Color colorN = updateColor(new Color(image.getRGB(texX, texY)), brightness);
