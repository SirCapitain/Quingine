...
public class Quomponent {
    ...
    public static double getDistance(Quisition pos1, Quisition pos2){
     ...
     */
    protected Quisition sizeCalc(double x, double y, double z){
        ...
        double delta = 2*Math.toDegrees(Math.atan((vec/(z-pic.getCameraPos().z))/2))/world.getFov();//Percent of how much the point needs to shrink by.
        return new Quisition(x+(x-quisition.x)*(delta-1), y+(y-quisition.y)*(delta-1), z+(z-quisition.z)*(delta-1));//The new points
    }
    public Quisition sizeCalc(Quisition quisition){
        return sizeCalc(quisition.x, quisition.y, quisition.z);
    }
