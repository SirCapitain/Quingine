...
public class Quobject extends Quomponent {
    ...
    protected Quisition transformQuisition(Quisition quisition){
        Quisition pos = new Quisition(0,0,quisition.z);
        if (pic != null && parentQuobject != null && isPerspective) {
            pos = parentQuobject.sizeCalc(quisition);
            pos.addQuisition(pic.getCameraPos());
        }else if (pic != null && isPerspective) {
            pos.x = sizeCalc(quisition).x + pic.getCameraPos().x;
            pos.y = sizeCalc(quisition).y + pic.getCameraPos().y;
        }else if (pic != null) {
            pos.x = quisition.x + pic.getCameraPos().x;
            pos.y = quisition.y + pic.getCameraPos().y;
        }else if (parentQuobject != null && isPerspective) {
            pos.x = parentQuobject.sizeCalc(quisition).x;
            pos.y = parentQuobject.sizeCalc(quisition).y;
        }else if (isPerspective){
            pos.x = sizeCalc(quisition).x;
            pos.y = sizeCalc(quisition).y;
        }else{
            pos.x = quisition.x;
            pos.y = quisition.y;
        }
    ...
    /**
     * You can't paint without pain.
     */
    public void draw() {
//        double theta = Math.atan((pic.getCameraPos().x - quisition.x)/(pic.getCameraPos().z - quisition.z));
        Quisition[] newPoints = new Quisition[points.length];
        for (int i = 0; i < points.length; i++)
            newPoints[i] = transformQuisition(points[i]);
        if (outline) {
             Quaphics.setColor(outlineColor);
             Quaphics.drawPolygon(newPoints, pic);
        }
        if (fill){
            Quaphics.setColor(fillColor);
            Quaphics.fillPolygon(newPoints,pic);
        }
    }
}
